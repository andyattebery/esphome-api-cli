...
async def switch_on(api_client: aioesphomeapi.APIClient, object_id: str) -> None:
    ...
    api_client.switch_command(entity_key, True)

async def switch_off(api_client: aioesphomeapi.APIClient, object_id: str) -> None:
    entity_key, assumed_state = await get_entity_key_state(api_client, object_id)
    api_client.switch_command(entity_key, False)
...
async def main():
    ...
    parser.add_argument('--object_id', type=str, help='object_id')
    ...
    switch_object_id = args.object_id if args.object_id is not None else "switch"

    if args.action == "switch_toggle":
        await switch_toggle(api_client, switch_object_id)
    elif args.action == "switch_off":
        await switch_off(api_client, switch_object_id)
    elif args.action == "switch_on":
        await switch_on(api_client, switch_object_id)
    else:
